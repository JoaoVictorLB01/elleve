...
export function useUserProgress(userId: string | undefined): ProgressStats {
  ...
  useEffect(() => {
    ...
    const fetchProgress = async () => {
      // Fetch enrollments and lesson progress in parallel
      ...
      // Calculate total hours from time_spent_seconds
      ...
      const courseDetails = enrollments.map(enrollment => {
        ...
        const completedInCourse = lessonProgress.filter(
          lp => lp.course_id === enrollment.course_id
        );
        ...
        if (course) {
          ...
          // If all completed, use last lesson
      ...
      // Overall progress: average of all enrolled courses
      const overallProgress = courseDetails.length > 0
        ? Math.round(courseDetails.reduce((sum, c) => sum + c.progress, 0) / courseDetails.length)
        : 0;

      // XP: 10 per completed lesson + 50 per certificate
