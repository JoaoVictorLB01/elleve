...
function Particles() {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      const i3 = i * 3;
      posArray[i3 + 1] += Math.sin(time * 0.25 + i * 0.1) * 0.0015;
      posArray[i3] += Math.cos(time * 0.15 + i * 0.15) * 0.0008;
    }
...
function FloatingOrbs() {
  ...
  useFrame((state) => {
    ...
    groupRef.current.children.forEach((child, i) => {
      const orb = orbs[i];
      if (!orb) return;
      child.position.y = orb.position[1] + Math.sin(time * orb.speed + i) * 0.4;
      child.position.x = orb.position[0] + Math.cos(time * orb.speed * 0.7 + i) * 0.25;
    });
  });
